...
/**
 * Enrich the harvested data with the broader document types by explicitly adding each broader type as a triple to the harvested triples dataset.
 * E.g. a 'Belastingsreglement' is also a 'Reglement en verordening'
 */
      ...
      ${cts.SPARQL_PREFIXES}
        ...
        WHERE {
          GRAPH <http://mu.semte.ch/graphs/public> {
            ${mu.sparqlEscapeUri(match.object.value)}
...
/**
 * Enrich the harvested data with the submitted publication URL such that the URL field in the form is automatically filled in. Note: the remoteFile is already persisted in the store by the automatic-submission service. We just need to enrich the harvested triples dataset that will be written to a TTL file.
 */
  ...
  if (bestuursorgaan) {
    const response = await mas.querySudo(`
      ...
      SELECT ?bestuursorgaanClassification ?bestuurseenheid ?bestuurseenheidClassification WHERE {
        GRAPH ?g {
          ${mu.sparqlEscapeUri(bestuursorgaan.value)}
...
async function expandDecisionToMeetingPath(store) {
  ...
  decisions.forEach((decision) => {
    const uuidBehandelingVanAgendapunt = mu.uuid();
        ...
        GRAPH <http://mu.semte.ch/graphs/public> {
          ...
          ?eenheid
            <http://data.vlaanderen.be/ns/besluit#classificatie>
              ${mu.sparqlEscapeUri(vgcClassificationUri)} .
        }
      }`);
    const sparqlJsonParser = new sjp.SparqlJsonParser();
