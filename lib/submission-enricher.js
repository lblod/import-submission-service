...
async function addClassifications(submittedDocument, triples) {
  ...
  if(isCentraalBestuurVanEredienstDocument(submittedDocument, triples)) {
    const bestuurseenheid = triples
        .filter(t => t.subject == submittedDocument && t.predicate == 'http://purl.org/dc/terms/creator')
          .map(t => t.object)[0];

    if(!bestuurseenheid) throw `Document must be corrupt; no bestuurseenheid found for ${submittedDocument}`;

    const result = await query(`
      PREFIX besluit: <http://data.vlaanderen.be/ns/besluit#>

      SELECT DISTINCT ?bestuurseenheid ?bestuurseenheidClassification WHERE {
        GRAPH ?g {
...
function isCentraalBestuurVanEredienstDocument(submittedDocument, triples) {
  const list = [
